...
export async function POST(req: NextRequest) {
  ...
  if (!res.ok) {
    ...
    return NextResponse.json({ error: "Failed to save submission", detail: err, baseId: process.env.AIRTABLE_BASE_ID ?? "MISSING", tableId: process.env.AIRTABLE_RENTAL_TABLE_ID ?? "MISSING" }, { status: 500 });
  }
